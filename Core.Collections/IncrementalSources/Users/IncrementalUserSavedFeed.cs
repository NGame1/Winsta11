...
{
    public class IncrementalUserSavedFeed : IIncrementalSource<InstaMedia>
    {
        PaginationParameters Pagination { get; }
        public long UserId { get; private set; } = -1;

        public IncrementalUserSavedFeed(long userId)
        {
        ...
        public async Task<IEnumerable<InstaMedia>> GetPagedItemsAsync(int pageIndex, int pageSize, CancellationToken cancellationToken = default)
        {
            ...
            {
                var result = await Api.FeedProcessor.GetSavedFeedAsync(Pagination, cancellationToken);
                if (!result.Succeeded)
                {
