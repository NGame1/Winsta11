...
namespace MinistaLivePlayback.Util
{
    public class DashManifestParser
    {
        ...
        private Windows.Data.Xml.Dom.XmlDocument document;
        private Manifest manifest;

        private bool hasPeriodStartAttribute;
        private TimeSpan periodStart;
        public DashManifestParser(Windows.Data.Xml.Dom.XmlDocument document, ref Manifest manifest)
        {
                ...
                foreach (var period in periods)
                {
                    ...
                    foreach (var adaptationSet in adaptationSets)
                    {
...
#endif
                        if (string.IsNullOrEmpty(stream.MimeType))
                        {
                            stream.MimeType = representation.Attributes.GetNamedItem("mimeType").InnerText;
                        }
                        ...
                        ParseRepresentationElement(representation,
                            out bitrate,
                            out repID);
                        // Period > AdaptationSet > Representation > SegmentTemplate > SegmentTimeline > S

                        var segmentTemplateQuery = representation.ChildNodes.Where(node => node.NodeName.Equals("SegmentTemplate"));
            ...
            string mediaFormat, ref MediaStream stream)
        {
            ...
            foreach (var t in timeline)
            {
                ...
                for (int i = 0; i < repeats + 1; i++)
                {
                    ...
                    ExpandDASHUrlSegmentTemplate(mediaFormat, manifest.BaseUrl, bitrate,
                        repID, segmentNumber, time, out mediaSegmentUrl);
                    segment.SegmentUrl = mediaSegmentUrl/*.Replace("/../","/")*/;
